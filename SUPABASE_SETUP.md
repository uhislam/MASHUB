...
### Create Usrah Table
```sql
CREATE TYPE usrah_gender AS ENUM ('Brother', 'Sister');
...
### Add Gender Field to Usrah (if upgrading existing database)
```sql
CREATE TYPE usrah_gender AS ENUM ('Brother', 'Sister');

ALTER TABLE usrah ADD COLUMN gender usrah_gender;
```
